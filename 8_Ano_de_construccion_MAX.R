...
median.construction.sii <- sii.extended %>% group_by(CodigoSIIdelaComuna,NumerodeManzana,NumerodePredial,CMN_MZ) %>% slice(which.max(Añodelalineadeconstruccion)) %>% group_by(CMN_MZ) %>% summarise(m=median(Añodelalineadeconstruccion))
